...
<div class="app-container calendar-list-container">
  ...
  <div class="filter-container">
    <el-input clearable class="filter-item" style="width: 200px;" placeholder="请输入优惠券标题" v-model="listQuery.param.title">
    </el-input>
  ...
  <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
    ...
    <el-table-column align="center" label="操作" width="150" class-name="small-padding fixed-width">
      <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleUpdate(scope.row)" v-show="scope.row.coupon.status === 0 ? true : false">编辑</el-button>
          <el-button type="danger" size="mini"  @click="handleDelete(scope.row)" v-show="scope.row.coupon.status === 0 ? true : false">下架</el-button>
          <el-button type="danger" v-show="scope.row.coupon.status === 1 ? true : false" disabled>已下架</el-button>
        </template>
    </el-table-column>

    <el-table-column align="center" width="50px" label="id" prop="coupon.id">
    </el-table-column>

    <el-table-column align="center" width="100px" label="优惠券标题" prop="coupon.title">
    </el-table-column>
  ...
  <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="60%">
    <el-form :rules="rules" ref="dataForm" :model="dataForm" status-icon label-position="left" label-width="100px" style='width: 300px; margin-left:50px;display:inline'>
      ...
      <el-form-item label="优惠券Logo">
        ...
        <el-upload action="#" :limit="1" :http-request="uploadPicUrls" :show-file-list="true" :file-list="dataForm.logoFileList" :before-upload="beforeAvatarUpload" list-type="picture-card" :on-preview="handlePictureCardPreview" :on-exceed="handleGoBeyond"
      ...
      <el-form-item label="优惠券种类" style="width:300px">
          <template slot-scope="scope">
              <el-radio-group v-model="dataForm.coupon.type" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="折扣" style="width:300px" v-show="dataForm.coupon.type == 1 ? true : false">
        <el-input v-model="dataForm.coupon.discount" placeholder="请输入0.1-9.9的折扣" :maxlength="inputMaxlength3" :disabled="dialogStatus == 'update' ? true:false">
            <template slot="append">折</template>
        </el-input>
      </el-form-item>

      <el-form-item label="减免金额" style="width:300px" v-show="dataForm.coupon.type == 0 ? true : false">
        <el-radio-group v-model="preferentialType" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
          <el-radio :label="0">无限制</el-radio>
          <el-radio :label="1">满减优惠</el-radio>
        </el-radio-group>
        <el-input v-model="dataForm.coupon.quota" placeholder="请输入满足金额" :maxlength="inputMaxlength6" v-show="preferentialType == 0 ? false : true" :disabled="dialogStatus == 'update' ? true:false">
          <template slot="prepend">满</template>
        </el-input>
        <el-input v-model="dataForm.coupon.money" placeholder="请输入减免金额" :maxlength="inputMaxlength6" v-show="dataForm.coupon.type == 0 ? true : false" :disabled="dialogStatus == 'update' ? true:false">
          <template slot="prepend">减</template>
        </el-input>
      </el-form-item>

      <el-form-item label="有效期" style="width:300px">
        <el-radio-group v-model="validType" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
        ...
        <el-input v-model="dataForm.coupon.validDay" placeholder="" style="width:250px" :maxlength="inputMaxlength6" v-show="validType == 1 ? true : false" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="使用限制" style="width:300px">
        <el-radio-group v-model="dataForm.coupon.isLimit" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
          <el-radio :label="0">无限制</el-radio>
        ...
        <el-input v-model="dataForm.coupon.goodsIds" placeholder="请输入多个商品id以逗号分隔" v-show="dataForm.coupon.isLimit == 1 ? true : false" :disabled="dialogStatus == 'update' ? true:false"></el-input>

        <el-checkbox-group v-model="dataForm.coupon.categoryList" v-show="dataForm.coupon.isLimit == 2 ? true : false"  style="display:flex;" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="领取方式" style="width:300px">
        <el-radio-group v-model="dataForm.coupon.receiveType" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="首页弹出" style="width:300px">
        <el-radio-group v-model="dataForm.coupon.isHomePopUpDisplay" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="首页显示" style="width:300px">
        <el-radio-group v-model="dataForm.coupon.isHomeDisplay" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="商品详情页显示" style="width:300px;display:-moz-stack">
        <el-radio-group v-model="dataForm.coupon.isDetailDisplay" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="商品分类显示" style="width:300px">
        <el-radio-group v-model="dataForm.coupon.isCategoryDisplay" style="display:inline-flex" :disabled="dialogStatus == 'update' ? true:false">
      ...
      <el-form-item label="每人限令" style="width:300px">
        <el-input v-model="dataForm.coupon.numLimit" placeholder="请输入每人限令数量" :maxlength="inputMaxlength8" :disabled="dialogStatus == 'update' ? true:false">
            <template slot="append">张</template>
        </el-input>
      </el-form-item>
      ...
      <el-form-item label="分享图片">
        <el-upload action="#" :limit="1" :http-request="uploadPicUrls2" :show-file-list="true" :file-list="dataForm.fileList2" :before-upload="beforeAvatarUpload" list-type="picture-card" :on-preview="handlePictureCardPreview" :on-exceed="handleGoBeyond"
          :on-remove="handlerOnRemove2" ref="upload2">
          <i class="el-icon-plus"></i>
        </el-upload>
      </el-form-item>
      ...
      <el-form-item label="上架">
        <el-radio v-model="dataForm.coupon.isOnSale" :label="1" :disabled="dialogStatus == 'update' ? true:false">立即上架</el-radio>
        <el-radio v-model="dataForm.coupon.isOnSale" :label="0" :disabled="dialogStatus == 'update' ? true:false">暂不上架</el-radio>
      </el-form-item>
...
export default {
  ...
  methods: {
    getList() {
      ...
      // if(this.listQuery.param.title == ""){
      //    this.listQuery.param.title = null
      // }
    ...
    hidePictureCardUpload() {
      if (this.dataForm.logoFileList.length >= 1) {
    ...
    handlerOnRemove(files, fileList) {
      this.dataForm.logoFileList = fileList
    ...
    uploadPicUrls(item) {
      ...
      uploadFile(formData).then(res => {
        ...
        this.dataForm.logoFileList.push(uploadUrlSuc)
        this.hidePictureCardUpload()
        ...
        this.dataForm.logoFileList.splice(this.dataForm.logoFileList.length, 1)
        console.log('item值\n' + JSON.stringify(this.dataForm.logoFileList, null, 2))
      })
    },
    ...
    uploadPicUrls2(item) {
      ...
      uploadFile(formData).then(res => {
        ...
        this.$message.error('上传失败，请重新上传')
        this.dataForm.fileList2.splice(this.dataForm.fileList2.length, 1)
    ...
    createData() {
      ...
      //设置领取数量
      this.dataForm.coupon.numSale = 0
      //设置剩余数量
      this.dataForm.coupon.numSurplus = this.dataForm.coupon.numTotal

      //赋值logo图片
      if(this.dataForm.logoFileList.length>0){
        this.dataForm.coupon.logo = this.dataForm.logoFileList[0].url
      }
      //赋值分享图片
    ...
    handleUpdate(row) {
      ...
      //this.dataForm = itemData 直接赋值的话相当于重新new了一个dataForm，底下的logoFileList和绑定的logoFileList就不是一个东西了

      this.dataForm.coupon.id = itemData.coupon.id
      this.dataForm.coupon.title = itemData.coupon.title
      this.dataForm.coupon.subtitle = itemData.coupon.subtitle
      this.dataForm.coupon.type = itemData.coupon.type
      this.dataForm.coupon.money = itemData.coupon.money
      this.dataForm.coupon.discount = itemData.coupon.discount
      this.dataForm.coupon.quota = itemData.coupon.quota
      this.dataForm.coupon.startTime = itemData.coupon.startTime
      this.dataForm.coupon.endTime = itemData.coupon.endTime
      this.dataForm.coupon.validDay = itemData.coupon.validDay
      this.dataForm.coupon.receiveType = itemData.coupon.receiveType
      this.dataForm.coupon.isWechat = itemData.coupon.isWechat
      this.dataForm.coupon.logo = itemData.coupon.logo
      this.dataForm.coupon.tag = itemData.coupon.tag
      this.dataForm.coupon.isLimit = itemData.coupon.isLimit
      this.dataForm.coupon.isHomePopUpDisplay = itemData.coupon.isHomePopUpDisplay
      this.dataForm.coupon.isHomeDisplay = itemData.coupon.isHomeDisplay
      this.dataForm.coupon.isDetailDisplay = itemData.coupon.isDetailDisplay
      this.dataForm.coupon.isCategoryDisplay = itemData.coupon.isCategoryDisplay
      this.dataForm.coupon.numTotal = itemData.coupon.numTotal
      this.dataForm.coupon.numLimit = itemData.coupon.numLimit
      this.dataForm.coupon.describe = itemData.coupon.describe
      this.dataForm.coupon.shareTitle = itemData.coupon.shareTitle
      this.dataForm.coupon.shareImage = itemData.coupon.shareImage
      this.dataForm.coupon.isOnSale = itemData.coupon.isOnSale
      this.dataForm.coupon.onsaleTime = itemData.coupon.onsaleTime
      this.dataForm.coupon.lowersaleTime = itemData.coupon.lowersaleTime


      this.dataForm.logoFileList = []
      ...
      if(itemData.coupon.logo != null){
          ...
          this.dataForm.logoFileList.push(imgData)
      }
      //设置分享图片
    ...
    updateData() {
          ...
          updateCoupon(this.dataForm.coupon).then(() => {
            this.dialogFormVisible = false
            this.getList()
            this.$notify({
              title: '成功',
    ...
    handleDelete(row) {
      ...
      this.$confirm('确定下架该优惠券吗?(下架后不可操作)', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteCoupon(row.coupon.id).then(response => {
          this.$notify({
            title: '成功',
            message: '下架成功',
            type: 'success',
