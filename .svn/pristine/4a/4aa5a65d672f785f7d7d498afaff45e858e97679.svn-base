...
<div class="app-container calendar-list-container">
  ...
  <div class="filter-container">
    ...
    <el-button class="filter-item" type="primary" @click="handlePreview()" icon="el-icon-mobile-phone">首页预览</el-button>
  ...
  <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
    ...
    <el-table-column type="expand" label="查看内容" width="100px">
      <template slot-scope="scope">
          <el-table label-position="left" :data="scope.row.recordValueList" border>
    ...
    <el-table-column align="center" width="150px" label="类型" prop="displayType">
      <template slot-scope="scope">
                                  <el-tag :type="scope.row.displayType === 0 ? 'success' : 'error' ">{{scope.row.displayType === 0 ? '商品分类' : '商品'}}</el-tag>
                        </template>
    </el-table-column>

    <el-table-column align="center" min-width="200px" label="名称" prop="name">
    </el-table-column>
  ...
  <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" @open="openDialog" :close-on-click-modal="false">
    <el-form :rules="rules" ref="dataForm" :model="dataForm" status-icon label-position="left" label-width="100px" style='width: 450px; margin-left:50px;'>
      ...
      <el-form-item label="类型" prop="selected">
        <div id="app">
      ...
      <el-form-item label="名称" prop="value9">
  ...
  <el-dialog :visible.sync="previewDialogVisible" width="500px">
    <div>
      ...
      <van-swipe class="goods-swipe" :autoplay="3000">
        <van-swipe-item v-for="item in list[0].recordValueList" :key="item.createTime">
          <img :src="item.imageUrl" width="460px">
        </van-swipe-item>
      </van-swipe>

      <van-row class="user-links">
        <van-col span="6" style="text-align:center" v-for="item in list[1].recordValueList" :key="item.createTime">
          <img :src="item.imageUrl" width="40px" height="40px"> <br>{{item.name}}
        </van-col>
      </van-row>

      <van-swipe class="goods-swipe" :autoplay="3000">
        <van-swipe-item v-for="item in list[2].recordValueList" :key="item.createTime">
          <img :src="item.imageUrl" width="460px">
        </van-swipe-item>
      </van-swipe>

      <van-cell-group class="user-group">
        <van-cell style="text-align: center;" title="精品专场" />
      </van-cell-group>

      <van-row class="user-links">
        <van-col span="6" style="text-align:center;padding:10px" v-for="item in list[3].recordValueList" :key="item.createTime">
      ...
      <van-cell-group class="user-group">
        <van-cell style="text-align: center;" title="特惠商品" />
      </van-cell-group>

      <van-row class="user-links">
        <van-col span="6" style="text-align:center;padding:10px" v-for="item in list[4].recordValueList" :key="item.createTime">
          <img :src="item.imageUrl" width="40px" height="40px">
          <br> {{item.name}}
        </van-col>
      </van-row>
      ...
      <van-cell-group class="user-group">
        <van-cell style="text-align: center;" title="新品推荐" />
      </van-cell-group>

      <van-row class="user-links">
        <van-col span="6" style="text-align:center;padding:10px" v-for="item in list[5].recordValueList" :key="item.createTime">
          <img :src="item.imageUrl" width="40px" height="40px">
          <br> {{item.name}}
        </van-col>
      </van-row>
      ...
      <van-cell-group class="user-group">
        <van-cell style="text-align: center;" title="为你推荐" />
      </van-cell-group>

      <van-row class="user-links">
        <van-col span="6" style="text-align:center;padding:10px" v-for="item in list[6].recordValueList" :key="item.createTime">
          <img :src="item.imageUrl" width="40px" height="40px">
          <br> {{item.name}}
        </van-col>
      </van-row>
      ...
      listGoodsNames(null).then(response => {
        ...
        if (this.dialogStatus == 'update' && this.listGoodsAndCay != null && up == 1) {
          ...
          for (var i = 0; i < this.options4.length; i++) {
            if (this.dataForm.displayId == this.options4[i].value) {
              this.value9 = this.options4[i].value
            }
          }
        }
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.dataForm.name = this.value9
          this.dataForm.displayId = this.value9
